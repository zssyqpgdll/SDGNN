...
class Train(object):
    ...
    def save_eval_model(self, running_avg_loss, iter):
        state = {
            'iter': iter,
        ...
        model_save_path = os.path.join(self.eval_dir, "best_checkpoint")
    ...
    def setup_train(self, model_file_path=None):
        ...
        # self.optimizer = AdamW(params, lr=initial_lr)
    ...
    def train_one_batch(self, batch):
        ...
        graph_enc_outputs, ast_encoder_outputs, graph_encoder_feature, ast_encoder_feature, graph_dec_in_state, \
            ast_dec_in_state = self.model.encoder(enc_batch, enc_lens, enc_graph_batch, enc_graph_mask_batch, enc_node_type_mask_batch)
        s_t_1 = self.model.reduce_state(graph_dec_in_state, ast_dec_in_state) # Tuple([1, b, h], [1, b, h])
        ...
        for di in range(min(max_dec_len, config.max_dec_steps)):
            ...
            final_dist, s_t_1,  c_t_1, attn_dist, p_gen, next_coverage = self.model.decoder(y_t_1, s_t_1,
                        ...
                        enc_padding_mask, c_t_1, extra_zeros, enc_batch_extend_vocab, coverage, di)
            target = target_batch[:, di]
            gold_probs = torch.gather(final_dist, 1, target.unsqueeze(1)).squeeze()
            step_loss = -torch.log(gold_probs + config.eps)
            if config.is_coverage:
    ...
    def trainIters(self, n_iters, model_file_path=None):
        ...
        while iter < n_iters:
            ...
            if iter % 5 == 0 and iter >= 1200:
                self.save_model(running_avg_loss, iter)

            # if iter % 5 == 0:
            #     if iter >= 1400:
            #         self.save_model(running_avg_loss, iter)
            #         print("save %d model, seconds for batch: %.2f" % (iter, time.time() - start))
            #         start = time.time()
            #
            #     print("start eval")
            ...
            #     eval_loss = self.eval_one_batch(eval_batch)
            #     eval_running_avg_loss = calc_running_avg_loss(eval_loss, eval_running_avg_loss, self.summary_writer, iter)
            #     print("eval_running_avg_loss: %.3f" % eval_running_avg_loss)
            #     if eval_best_loss is None or eval_running_avg_loss < eval_best_loss:
            #         print("Found new best model with %.3f running_avg_loss at %d. Saving to %s" % (
            #             eval_running_avg_loss, iter, self.eval_dir))
            #         self.save_eval_model(eval_running_avg_loss, iter)
            #         eval_best_loss = running_avg_loss
