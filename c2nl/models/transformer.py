...
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        #self.node_type_index_embeddings = nn.Embedding(num_embeddings=args.num_node_type + 1, embedding_dim=self.enc_input_size, padding_idx=0)
    ...
    def forward(self,
                sequence,
                sequence_char,
                sequence_nodex_type_index=None,
                sequence_type=None,
                mode='encoder',
                step=None):
        ...
        if mode == 'encoder':
            ...
            #node_type_index_rep = self.node_type_index_embeddings(sequence_nodex_type_index)
            #word_rep = word_rep + node_type_index_rep
...
class Encoder(nn.Module):
    ...
    def forward(self,
                ...
                input_len,
                adjacency,
                code_node_type_mask_rep):
        layer_outputs, _ = self.transformer(input, input_len, adjacency, code_node_type_mask_rep)  # B x seq_len x h
        if self.use_all_enc_layers:
...
class CustomModel(nn.Module):
    ...
    def _run_forward_ml(self,
        ...
        code_node_type_index_rep = kwargs['code_node_type_index_rep']
        code_node_type_mask_rep = kwargs['code_node_type_mask_rep']

        code_rep = self.embedder(code_word_rep,
                                 code_char_rep,
                                 code_node_type_index_rep,
                                 code_type_rep,
                                 mode='encoder')
        code_struc_rep = kwargs['code_struc_rep']


        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len, code_struc_rep, code_node_type_mask_rep)  # B x seq_len x h
    ...
    def decode(self,
        ...
        memory_bank, layer_wise_outputs = self.encoder(word_rep, code_len, code_struc_rep, code_node_type_mask_rep)  # B x seq_len x h
