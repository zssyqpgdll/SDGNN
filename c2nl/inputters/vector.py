...
def vectorize(ex, model):
    ...
    vectorized_ex['code_node_type_index_rep'] = torch.LongTensor(code.node_type_index)
    vectorized_ex['code_node_type_mask_rep'] = torch.LongTensor(code.node_type_mask)
...
def batchify(batch):
    ...
    code_node_type_indexs = [ex['code_node_type_index_rep'] for ex in batch]
    code_node_type_masks = [ex['code_node_type_mask_rep'] for ex in batch]
    ...
    code_node_type_index_rep = torch.zeros(batch_size, max_code_len, dtype=torch.long)

    num_node_type = code_node_type_masks[0].size(0)
    code_node_type_mask_rep = torch.zeros(batch_size, num_node_type, max_code_len, 1, dtype=torch.long)
    ...
    for i in range(batch_size):
        ...
        code_node_type_index_rep[i, :code_node_type_indexs[i].size(0)].copy_(code_node_type_indexs[i])
        code_node_type_mask_rep[i, :code_node_type_masks[i].size(0), :code_node_type_masks[i].size(1), :code_node_type_masks[i].size(2)].copy_(code_node_type_masks[i])
    ...
    return {
        ...
        'code_node_type_index_rep': code_node_type_index_rep,
        'code_node_type_mask_rep': code_node_type_mask_rep,
